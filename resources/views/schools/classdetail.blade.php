            ...
            <div class="tab-content">
                    ...
                    @else
                        <table class="table  table-green table-student tablesorter" id="studentsTable">
                            ...
                            @foreach ($section->getStudents() as $student)
                                <tr class="tr-content">
                                    <td><img src="../../public/images/header-default.jpg" realheader="/userPhoto/{{$student->o365UserId}}"/> {{$student->displayName}} </td>
                        ...
                        <table id="stuconv" class="table  table-green table-student">
                            ...
                            @foreach ($conversations as $conversation)
                                <tr class="tr-content">
                                    ...
                                    <td class="space-holder left"></td>
                                    <td>
                                        <a target="_blank" href="{{$seeMoreConversationsUrl . '&ConvID=' . $conversation->getId()}}">{{$conversation->getTopic()}}</a>
                                    </td>
                                    <td class="space-holder right"></td>
                                </tr>
                            @endforeach
                ...
                <div id="seatingchart" class="tab-pane fade ">
                    <div class="left" id="dvleft">
                        ...
                        @else
                            <div class="assignseat tip"><div class="greenicon" style="background-color:{{$myFavoriteColor}}"></div>Your assigned seat </div>
                        @endif
                        <ul id="lstproducts">
                            @foreach ($section->getStudents() as $student)
                                <li id="{{$student->o365UserId}}"><img src="../../public/images/header-default.jpg" realheader="/userPhoto/{{$student->o365UserId}}" /> <span class="disname"> {{$student->displayName}} </span> <span class="seated {{$student->position === 0 ? 'hideitem' : ''}}">seated ✓</span> </li>
                            @endforeach
                        </ul>
                        <div id="hidtiles" class="hideitem">
                            @foreach ($section->getStudents() as $student)
                                <div class="deskcontainer {{$student->position === 0 ? 'unsaved': 'white'}} {{empty($student->favoriteColor) ? '': 'green'}}" style="{{empty($student->favoriteColor) ? '': 'background-color:' . $student->favoriteColor}}" position="{{$student->position}}" userid="{{$student->o365UserId}}">
                                    <div class="deskclose"><img src="../../public/Images/close.png"></div>
                                    <div class="deskicon">
                                        <img src="/images/header-default.jpg" realheader="/userPhoto/{{$student->o365UserId}}" />
                                    </div>
                    ...
                    <div class="right" id="dvright">
                        <div class="teacherdesk" style="{{$isStudent ? '' : 'background-color:' . $myFavoriteColor}}">Teacher's desk</div>
                        <div>
                            <div id="graybg">
                                @for ($i = 1; $i <= 36; $i++)
